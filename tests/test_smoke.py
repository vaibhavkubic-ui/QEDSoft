...
class QEDSoftSmokeTest(unittest.TestCase):
    def test_fifo_static_flow(self) -> None:
        ...
        matlab = root / "examples" / "matlab" / "fifo_step.m"
        ...
        self.assertTrue(any(artifact.kind == "lean_matlab" for artifact in result.artifacts))
        self.assertTrue(any(artifact.kind == "lean_hdl" for artifact in result.artifacts))
        self.assertTrue(any(artifact.kind == "lean_equivalence" for artifact in result.artifacts))
